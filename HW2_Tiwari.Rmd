...
output:
  html_document:
    df_print: paged
---
Collaborators: No collaborators for coding

Citation: Lecture notes
...
```{r}
est <- read_csv("est.csv",show_col_types = FALSE)
contra <-read_csv("contraceptive_use.csv", show_col_types = FALSE)

dat<-contra%>%
  rename(iso=division_numeric_code)%>%
  mutate(cpr_year= round((start_date + end_date)/2))%>%
  mutate(mCPR=contraceptive_use_modern*100)%>%
...
```{r}
library(devtools)
load_all("/Users/aparnatiwari/Documents/R 632/PlotCp/Plotsfunc")
```
...
```{r}
document("/Users/aparnatiwari/Documents/R 632/PlotCp/Plotsfunc")
```

```{r, warning=FALSE}
library(Rd2md)
Rd2markdown(rdfile='//Users/aparnatiwari/Documents/R 632/PlotCp/Plotsfunc/man/plot_cp.Rd', outfile='/Users/aparnatiwari/Documents/R 632/PlotCp/Plotsfunc/plot_cp.md')
```
